...
def get_initial_sample(spark, ds_dash):
    ...
    df = spark.sql(
        f"""
                    SELECT
                        client_id,
                        active_addons as addon_data
                    FROM
                        `moz-fx-data-shared-prod`.telemetry.clients_last_seen as tbl
                    WHERE
                        active_addons IS NOT null and
                        array_length(active_addons) > 1 and
                        channel = 'release' and
                        normalized_channel = 'release' and
                        app_name = 'Firefox' and
                        tbl.submission_date = '{ds_dash}'
                     """
    )
...
def extract_telemetry(spark, ds_dash):
    ...
    client_features_frame = get_initial_sample(spark, ds_dash)
...
def load_amo_external_whitelist():
    ...
    amo_dump = read_from_gcs(
        AMO_WHITELIST_FNAME, AMO_WHITELIST_PREFIX, AMO_DUMP_BUCKET
    )
...
def main(date, aws_access_key_id, aws_secret_access_key, bucket, prefix):
    thedate = dt.datetime.strptime(date, "%Y%m%d").date()
    ...
    longitudinal_addons = extract_telemetry(spark, thedate.strftime("%Y-%m-%d"))
    result_df = transform(longitudinal_addons)
    load_s3(result_df, thedate.strftime("%Y%m%d"), prefix, bucket)
